rooms.example3D = function () {
  ...
  code = {
   ...
   for (let n = 0; n < S.nS; n++) {
      S.sc.push(rnd(), rnd(), rnd(), .2); // create an array of 10 random spheres
   }
`,
    vertex: `
S.setVertexShader(\`
...
S.setFragmentShader(\`
  ...
  vec3 shadeSphere(vec3 P, vec4 S) {
    ...
    for (int l = 0; l < nL; l++) {
      ...
      // are we shadowed by other sphere?
      float t = -1.;
      ...
      // if not, add lighting from this light source
      if (t < 0.) {
  ...
  void main() {
      ...
      // 2 spheres - changed to uS and setUniform
