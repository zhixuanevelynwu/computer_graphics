...
`,
  ...
  let walkSpeed = mix(.5, 1.5, walk_speed.value / 100);

  let stepLength = step_length.value / 100;
  stepLength = .5 + stepLength;
  stepLength /= walkSpeed;

  let feetApart = feet_apart.value / 100;

  let stepWeight = mix(.1, 1, step_weight.value / 100);
  ...
  let resample = C => {
     ...
     for (let t = 0 ; t < T ; t += T/N)
        R.push({t: t/walkSpeed, p: sample(C, t)});
     return R;
  }
  ...
  if (S.isWalk) {
     ...
     let N = Math.floor(100 * stepLength);
	...
	let foot = (f,color) => {
	   ...
	   let footPosition = i => {
              D = S.scale(S.normalize(S.subtract(C[i].p, C[i-1].p)),
	                  20*stepLength*walkSpeed);
              let p = S.normalize(S.cross(D, [0,0,1]));
              return S.add(C[i].p, S.scale(p, s * (.2 + .8 * feetApart) * 100));
	   }

           let p = footPosition(i);
           S.context.strokeStyle = color;
           S.context.lineWidth = 20 * stepWeight;
	      ...
	      S.feet3D[S.feet3D.length-1].push([fp[0], fp[1], 50*z]);
	   }
	}
	for (let f = 0 ; f < 2 ; f++) {
     ...
     let b = C[iFeet].p;
     S.body = mix(S.body, C[iFeet].p, .125 / stepLength);
     ...
     if (S.isFeet) {
        ...
        S.bodyLift = mix(S.bodyLift, (z0 + z1) / 2, .5);
        S.body3D.push([S.body[0], S.body[1], 200 + S.bodyLift]);
     }

     let ds = (2 + stepWeight) * walkSpeed;
