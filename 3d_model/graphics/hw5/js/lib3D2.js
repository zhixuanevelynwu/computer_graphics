...
`;
  ...
  addEventListenersToCanvas = function (canvas) {
    ...
    window.addEventListener(
      "keyup",
      function (e) {
        if (!isEditing()) onKeyRelease(e.keyCode);
      },
      true
    );
  };
    ...
    setTimeout(function () {
      ...
      canvas.setShaders = function (vertexShader, fragmentShader) {
        ...
        function addshader(type, src) {
          ...
          } else gl.attachShader(program, shader);
        }
  ...
  let defaultVS =
    "attribute vec3 aPos;varying vec3 vPos;void main(){gl_Position=vec4(vPos=aPos,1.);}";
  let defaultFS =
    "varying vec3 vPos;void main(){gl_FragColor=vec4(sqrt(vPos),1.);}";

  let vertexShader = defaultVS,
