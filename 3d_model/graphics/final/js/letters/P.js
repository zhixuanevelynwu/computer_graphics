window.PDiagram = (cellX = 0, cellY = 0) => {
  designP(cellX, cellY);
  normalP();
  boldP();
  italicP();
...
let boldP = () => {
  ...
  diagram.setLineWidth(Math.max(10, (S.wP / 2) * 3));
...
let designP = (cellX, cellY) => {
  ...
  if (diagram.isPress()) {
    for (let k = 0; k < K.length - 1; k++) {
      if (isNear(K[k])) S.indexP = k;
    }
    for (let k = 0; k < K0.length; k++) {
      if (isNear(K0[k])) S.indexP0 = k;
    }
  }
  if (S.indexP >= 0) {
    let p = [x, y];
  ...
  if (diagram.isPress() && isNear(d))
    S.tSlideP = y >= d[1] - 10 && y < d[1] + 10;
  if (diagram.isPress() && isNear(d1))
    S.wSlideP = y >= d1[1] - 10 && y < d1[1] + 10;

  if (S.tSlideP) {
    S.tP = t = Math.max(0, Math.min(1, (x - cellX + 50) / 360));
  }

  if (S.wSlideP) {
    S.wP = w = Math.max(1, Math.min(1, (x - cellX + 40) / 360) * 20);
  }
