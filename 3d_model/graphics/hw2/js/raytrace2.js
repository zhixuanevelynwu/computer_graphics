rooms.raytrace2 = function () {
  ...
  description = `Raytracing to spheres<br>in a fragment shader<br>(PHONG Shading & <br>Reflection added)`;
...
*****************************************************/
            // PART 1
            c += uLc[l] * (Diffuse * max(0., dot(N, uLd[l])) + Specular.xyz * pow(max(0., dot(R,-W)), Specular.w));
...
*****************************************************/
            ...
            vec3 R = 2. * dot(N, -W) * N + W; // direction
            color += uLc[n] * (uSm[n][1].rgb * max(0., dot(N, uLd[n])) + uSm[n][2].xyz * pow(max(0., dot(R,-W)), uSm[n][2].w));
            */
            vec3 N = normalize(P - uS[n].xyz);
            vec3 R = 2. * dot(N, -W) * N + W; // direction
            ...
            float tMinR = 10000.;
            for (int l = 0; l < nS; l++) {
              float tR = raySphere(P, R, uS[l]);
              if (tR > 0. && tR < tMinR) {
                color += shadeSphere(P, R, uS[l], uSm[l]) * uSm[n][2].rgb;
                tMinR = tR;
              }
            }
...
`,
   ...
   // make circular movements instead
   S.sc[3] = [ c*.3 + .3,       s*.3 + .3,    .5, .3, 0,0,0];
   S.sc[1] = [ -c*.2,            s*.2,         0.,  .8, 0,0,0]; 
   S.sc[0] = [ c*.1 - .3,       s*.1 - .3,    -.5, .3, 0,0,0];
   S.sc[2] = [ -c*.01 - .6,       s*.01 - .6,    -1., .3, 0,.8,0];

   radius.defaultValue = 0;
